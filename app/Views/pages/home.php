...
<section id="home" class="py-32 bg-gradient-to-br from-slate-50 via-blue-50/40 to-indigo-50/30 relative overflow-hidden section-enterprise-premium">
    ...
    <div class="absolute inset-0 z-0">
        ...
        <div class="floating-3d-element floating-sphere absolute top-20 left-16"></div>
        <div class="floating-3d-element floating-cube absolute top-1/4 right-20"></div>
        <div class="floating-3d-element floating-cylinder absolute bottom-1/3 left-1/3"></div>
        <div class="floating-3d-element floating-torus absolute top-2/3 right-1/4"></div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 relative z-10">
        <div class="text-center space-y-16" data-aos="fade-up">
            ...
            <div class="flex flex-col sm:flex-row justify-center gap-6 pt-4">
                <a href="<?= base_url('login') ?>" class="group px-12 py-5 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-2xl font-semibold hover-lift inline-flex items-center justify-center transition-all duration-300">
            ...
            particlesJS('particles-js', {
                particles: {
                    ...
                    move: {
                        ...
                        out_mode: 'bounce',
                        bounce: true,
                        attract: { enable: true, rotateX: 3000, rotateY: 2500 }
                    }
                },
                interactivity: {
                    detect_on: 'canvas',
            ...
            const updateParticles = () => {
                if (window.pJSDom && window.pJSDom[0]) {
                    ...
                    particles.forEach((particle, index) => {
                        // Rocket launch acceleration effect
                        const boost = Math.sin(Date.now() * 0.001 + index * 0.1) * 0.5 + 0.5;
                        particle.vx += Math.cos(index * 0.2) * boost * 0.1;
                        particle.vy -= boost * 0.05; // Upward bias

                        // Implosion effect every few seconds
                        if (index % 50 === Math.floor(Date.now() * 0.01) % 50) {
    ...
    if (typeof THREE !== 'undefined') {
        const initSpaceXBackground = function() {
            ...
            const launchSequence = () => {
                ...
                // Animate satellites in orbital mechanics
                satellites.forEach((satellite, index) => {
                    const currentTime = time * satellite.userData.orbitalSpeed;
